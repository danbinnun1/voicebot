...
progressFileContent = ''
with open(Data._progressFilePath, 'r') as file:
...
def signMember(name):
    ...
    with open(Data._progressFilePath, 'a+') as file:
        file.write(name+','+Tone.firstTone()+'\n')
    os.mkdir(Data._recordingsFolderPath+'/'+name)
    file.close()
    ...
    if tonesOrder == -1:
        ...
        for row in rows:
            if row.split(',')[0] == memberName:
                ...
                newRows.append(row)
        with open(Data._progressFilePath, 'w') as file:
            for row in newRows[0:len(rows)-1]:
                file.write(row+'\n')
        file.close()
    else:
        os.remove(Data.temporalFilePath(recording))
        raise SoundException(
